...
describe('Creating application', () => {
  ...
  it.only('Creates an application and redirects to application', () => {
    ...
    //.type('     ' + baseQuery);//Possible solution to delay
    ...
    cy.get('.euiTableRow').should('have.length.lessThan', 2);//Replaces Wait
  ...
  it('Saves current input on reload', () => {
    ...
    cy.wait(delay);//Dropdown menu needs to work properly
    ...
    cy.get('[data-test-subj="logSourceAccordion"]').trigger('mouseover').click();
    cy.get('[data-test-subj="searchAutocompleteTextArea"]').should('contain.value', baseQuery, {delay: TYPING_DELAY});
    cy.get('[data-test-subj="servicesEntitiesCountBadge"]').should('contain', '1');
    cy.get('[data-test-subj="traceGroupsCountBadge"]').should('contain', '2');
  });
  ...
  it('Saves time range for each application', () => {
    cy.wait(delay);//Fail to load without
    ...
    cy.get('.euiTableRow').should('have.length.greaterThan', 1);//Replaces wait
...
describe('Viewing application', () => {
  ...
  it.only('Saves visualization #1 to panel', () => {
    cy.get('[data-test-subj="app-analytics-panelTab"]').click();
  ...
  it.only('Saves visualization #2 to panel with availability level', () => {
    changeTimeTo24('months');
    cy.get('[data-test-subj="app-analytics-logTab"]').click();
  ...
  it('Configuration tab shows details', () => {
    ...
    cy.get('option').should('have.length', 2);//ADAM ERROR
  });
  ...
  it('Changes availability visualization', () => {
    cy.get('[data-test-subj="app-analytics-configTab"]').click();
    cy.wait(delay);
    cy.get('select').select(visOneName);//ADAM ERROR
    cy.wait(delay);
    moveToHomePage();
